        ...
        for seat_type in seats.keys():
            ...
            if self.filter_train:  # 开启了过滤车次
                if self.filter_train.get(ticket_info['trainNo']):  # 当前车次是过滤车次
                    if len(self.filter_train.get(ticket_info['trainNo']).keys()) != 0:  # 余票低于阈值监控
...
if __name__ == '__main__':
    ...
    # TicketMonitor.start([
    #     {'from': '南阳', 'to': '上海', 'date': '2017-10-07'}])
    # {'from': 'SHH', 'to': 'NFF', 'date': '2017-09-28'},
    # {'from': 'SHH', 'to': 'NFF', 'date': '2017-09-29'}])
